...
///
/// - Если счетчик strong ссылок становится равен нулю, то объект всегда переходит в состояние deiniting.
///   Unowned ссылки выкидывают ошибку в runtime, а чтение weak ссылок возвращает nil.
///
/// - Счетчик unowned ссылок получает +1 от счетчика strong ссылок,
///   который впоследствие уменьшается после завершения функции deinit() объекта.
///
/// - Счетчик weak ссылок получает +1 от счетчика unowned ссылок.
///   Он уменьшается после освобождения (freed) объекта из памяти.
